...
Feature: ADR 009 – Discovery of AAS Services via DSP
  ...
  Scenario: Provider advertises Digital Twin Registry as Dataset in DSP Catalog
    ...
    And the Dataset has annotation "http://purl.org/dc/terms/type" equal to "https://w3id.org/catenax/taxonomy#DigitalTwinRegistry"
  ...
  Scenario: subprotocolBody key-value pairs are correctly parsed with "=" assignment and ";" separator
    Given a SubmodelDescriptor endpoint has a "subprotocolBody" value "id=my-dataset-id;dspEndpoint=https://provider.example.com/protocol"
  ...
  Scenario: subprotocolBody is malformed (missing required key)
    Given a SubmodelDescriptor endpoint has subprotocolBody "dspEndpoint=https://provider.example.com" (missing the "id" key)
    When a Consumer parses the subprotocolBody
