...
Feature: ADR 008 – Asset Administration Shell Profile for Factory-X
  ...
  Scenario: Provider exposes AAS Registry and Discovery from a common URL path
    ...
    Then both the "AssetAdministrationShellRegistryServiceSpecification-SSP2" and the "DiscoveryServiceSpecification-SSP1" endpoints are served from the same URL path
